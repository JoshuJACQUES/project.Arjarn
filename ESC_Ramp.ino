...
ESC myESCL (13, SPEED_MIN, SPEED_MAX, 500);   
ESC myESCR (15, SPEED_MIN, SPEED_MAX, 500);                 // ESC_Name (ESC PIN, Minimum Value, Maximum Value, Default Speed, Arm Value)
...
void setup() {
  ...
  myESCL.arm(); 
  myESCR.arm();                                                   // Send the Arm value so the ESC will be ready to take commands
...
void loop() {
  for (oESC = SPEED_MIN; oESC <= SPEED_MAX; oESC += 1) {  // goes from 1000 microseconds to 2000 microseconds
    myESCR.speed(oESC); 
    myESCL.speed(oESC); 
    Serial.println(oESC);                                  // tell ESC to go to the oESC speed value
